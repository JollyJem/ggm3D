...
- The ggmgastro nameplate is a textured two-triangle quad recessed into the front rim (`parts.nameplate_face` / `nameplate_recess`, texture from `scripts/make_badge.py`). Recessed rather than proud, so the product stays exactly as deep as the catalog says. trimesh keeps UVs v-up and flips them on glTF export, so a UV written the glTF way lands the wordmark upside down — there is a test for that.
...
- One mesh per material, so a product exports as three nodes however many boxes went into it, plus the nameplate quad — the one part whose material carries a texture and so cannot be merged. Scene Viewer pays per node.
