...
FOOT_R = 18.0
# The riveted nameplate, on the front rim at the right-hand end, where the
# product photo has it. 150 mm reads at 4:1, the texture's own aspect.
BADGE_W = 150.0
BADGE_ASPECT = 4.0
BADGE_MARGIN = 60.0  # from the right end of the unit
...
def to_scene(
    ...
    badge: trimesh.Trimesh | None = None,
) -> trimesh.Scene:
    ...
    if badge is not None:
        # added after the loop, and never merged into a group: it is the one
        # part whose material carries a texture, and its UVs have to survive
        badge.apply_transform(rot)
        badge.apply_scale(0.001)
        scene.add_geometry(badge, geom_name="badge")
    return scene
...
def _badge_placement(
    w: float, rz0: float, rz1: float
) -> tuple[float, float, tuple[float, float, float]]:
    """Size and centre of the nameplate on the front rim, in build mm.

    Right-hand end of the rim, the corner the photo shows it in, standing off
    the end by BADGE_MARGIN. Both the plate and that margin shrink with a
    narrow unit so the plate can never run off the end of the rim it sits on.
    """
    plate_w = min(BADGE_W, w * 0.2)
    plate_h = plate_w / BADGE_ASPECT
    margin = min(BADGE_MARGIN, w * 0.03)
    return plate_w, plate_h, (w - margin - plate_w / 2, 0.0, (rz0 + rz1) / 2)


def badge_mesh(w: float, rz0: float, rz1: float) -> trimesh.Trimesh:
    """The nameplate quad itself, textured with the wordmark."""
    plate_w, plate_h, center = _badge_placement(w, rz0, rz1)
    verts, faces, uv = parts.nameplate_face(plate_w, plate_h, center)
    mesh = trimesh.Trimesh(vertices=verts, faces=faces, process=False)
    mesh.visual = TextureVisuals(uv=uv, material=nameplate())
    return mesh
...
def _dbl_worktop(
    ...
    front_rim = parts.box_from_bounds(0, w, 0, 15, rz0, rz1)
    plate_w, plate_h, plate_center = _badge_placement(w, rz0, rz1)
    front_rim = parts.nameplate_recess(front_rim, plate_w, plate_h, plate_center)
    steel += [
        front_rim,
...
def build_double_sink(spec: BuildSpec) -> trimesh.Scene:
    ...
    rim_top = top_z - DBL_WORKTOP_T
    badge = badge_mesh(w, rim_top - DBL_RIM_H, rim_top)
    for mesh in [*steel, *plastic, *worktop, badge]:
        mesh.apply_translation((-w / 2, -d / 2, 0.0))  # origin to floor center
    return to_scene(steel, plastic, worktop, badge)
